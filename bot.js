...
// --- NEW GLOBAL CONSTANT ---
const KEYBOARD_VERSION = 2; // Increment this number for every new keyboard update
...
app.post('/api/bots/restart', validateWebAppInitData, async (req, res) => {
    ...
    try {
        ...
        console.error(`[MiniApp] Error restarting bot ${appName}:`, e.message);
...
app.post('/api/bots/redeploy', validateWebAppInitData, async (req, res) => {
    ...
    try {
        ...
        console.error(`[MiniApp] Error redeploying bot ${appName}:`, e.message);
        res.status(500).json({ success: false, message: 'Failed to redeploy.' });
    }
});

// POST /api/bots/delete - Delete a bot
app.post('/api/bots/delete', validateWebAppInitData, async (req, res) => {
    ...
    try {
        ...
        // First, delete from Heroku
        await axios.delete(`https://api.heroku.com/apps/${appName}`, {
            headers: { Authorization: `Bearer ${HEROKU_API_KEY}`, Accept: 'application/vnd.heroku+json; version=3' }
        });
        // Then, clean up the database records
        await dbServices.deleteUserBot(userId, appName);
        await dbServices.markDeploymentDeletedFromHeroku(userId, appName);
        res.json({ success: true, message: 'Bot successfully deleted.' });
    } catch (e) {
        console.error(`[MiniApp] Error deleting bot ${appName}:`, e.message);
        res.status(500).json({ success: false, message: 'Failed to delete bot.' });
    }
});

  // GET /api/miniapp/opened - Notify admin when a user opens the web app
app.get('/api/miniapp/opened', validateWebAppInitData, async (req, res) => {
    try {
        const userId = req.telegramData.id.toString();
        const userDetails = req.telegramData;
        const msg = {
            from: {
                id: userId,
