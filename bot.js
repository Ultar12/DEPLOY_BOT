...
function buildKeyboard(isAdmin) {
    const baseMenu = [['Get Session', 'Deploy'], ['Free Trial', 'My Bots'], ['Restore', 'Support']];
    if (isAdmin) { return [['Deploy', 'Apps'], ['Generate Key', 'Get Session'], ['Restore', 'Support']]; }
    return baseMenu;
}
function chunkArray(arr, size) { const out = []; for (let i = 0; i < arr.length; i += size) { out.push(arr.slice(i, i + size)); } return out; }
async function sendAnimatedMessage(chatId, baseText) { const msg = await bot.sendMessage(chatId, `⚙️ ${baseText}...`); await new Promise(r => setTimeout(r, 1200)); return msg; }
async function turnOffDyno(appName) { try { await axios.patch(`https://api.heroku.com/apps/${appName}/formation/web`, { quantity: 0 }, { headers: { Authorization: `Bearer ${HEROKU_API_KEY}`, Accept: 'application/vnd.heroku+json; version=3', 'Content-Type': 'application/json' } }); console.log(`Successfully scaled down dyno for ${appName}.`); return true; } catch (error) { console.error(`Failed to scale down dyno for ${appName}:`, error.response?.data?.message || error.message); return false; } }
        ...
        if (status === 'succeeded') {
            ...
            if (isFreeTrial) {
                ...
                const countdownInterval = setInterval(async () => {
                    minutesLeft -= 5;
                    if (minutesLeft > 0) {
                        bot.editMessageText(`✅ Your bot is live!\nhttps://${name}.herokuapp.com\n\n⏳ *Trial time left: ${minutesLeft} minutes*`, { chat_id: chatId, message_id: liveMessage.message_id, parse_mode: 'Markdown' }).catch(() => clearInterval(countdownInterval));
                    } else {
                        ...
                        bot.sendMessage(ADMIN_ID, `*Trial Expired & Dyno Off*\n\nApp Name: \`${name}\`\nUser ID: \`${chatId}\`\n\nThe app has been turned off. Please delete it.`, { parse_mode: 'Markdown', reply_markup: { inline_keyboard: [[{ text: "🗑️ Delete Now", callback_data: `admindelete:${name}:${chatId}` }]] } }).catch(err => console.error("Failed to send deletion request to admin:", err));
                    }
                }, 5 * 60 * 1000);
            }
            return true;
...
// 10) Command handlers
bot.onText(/^\/start$/, async msg => { const cid = msg.chat.id.toString(); const isAdmin = cid === ADMIN_ID; delete userStates[cid]; await bot.sendMessage(cid, isAdmin ? 'Welcome, Admin! Here is your menu:' : 'Welcome! Please select an option:', { reply_markup: { keyboard: buildKeyboard(isAdmin), resize_keyboard: true } }); });
bot.onText(/^\/menu$/i, msg => { const cid = msg.chat.id.toString(); const isAdmin = cid === ADMIN_ID; bot.sendMessage(cid, 'Menu:', { reply_markup: { keyboard: buildKeyboard(isAdmin), resize_keyboard: true } }); });
bot.onText(/^\/apps$/i, msg => { if (msg.chat.id.toString() === ADMIN_ID) { bot.sendMessage(msg.chat.id.toString(), "Admin app list functionality is handled via Heroku dashboard or CLI."); } });
...
bot.on('message', async msg => {
  ...
  // --- RESTRUCTURED LOGIC ---
  // FIRST, check for button presses IF the user is NOT in a stateful flow
  if (!st) {
    if (text === 'Deploy') {
    ...
    } else if (text === 'Get Session') {
        const guideCaption = "To get your session ID, please follow these steps carefully:\n\n1️⃣ *Open the Link*\nVisit: https://levanter-delta.vercel.app/\n\n2️⃣ *Important for iPhone Users*\nIf you are on an iPhone, please open the link using the **Google Chrome** browser for best results.\n\n3️⃣ *Skip Advertisements*\nThe website may show ads. Please close or skip any popups or advertisements to proceed.\n\n4️⃣ *Use a CUSTOM ID*\nYou **must** enter your own unique ID in the 'Custom Session' field. Do not use the default one. A good ID could be your name or username (e.g., `johnsmith`).\n\nOnce you have copied your session ID, tap the 'Deploy' button here to continue.";
        try { await bot.sendPhoto(cid, 'https://files.catbox.moe/an2cc1.jpeg', { caption: guideCaption, parse_mode: 'Markdown' }); }
        catch { await bot.sendMessage(cid, guideCaption, { parse_mode: 'Markdown' }); }
    } else if (text === 'My Bots') {
        const bots = await getUserBots(cid);
        if (!bots.length) { bot.sendMessage(cid, "You haven't deployed any bots yet."); }
        else { const rows = chunkArray(bots, 3).map(r => r.map(n => ({ text: n, callback_data: `selectbot:${n}` }))); bot.sendMessage(cid, 'Your deployed bots:', { reply_markup: { inline_keyboard: rows } }); }
    } else if (text === 'Support') {
        bot.sendMessage(cid, `For help, contact the admin: ${SUPPORT_USERNAME}`);
    }
  // SECOND, if it's not a button press, handle the stateful flows
  } else {
    if (st.step === 'AWAITING_KEY') {
        ...
        st.step = 'SESSION_ID';
        delete userStates[cid].data.isFreeTrial; // No longer needed
    ...
    if (st.step === 'AUTO_STATUS_VIEW') {
        ...
        if (buildSuccessful) { await addUserBot(cid, APP_NAME, SESSION_ID, isFreeTrial); }
    ...
    if (st.step === 'RESTORE_SESSION_ID') {
        ...
        try {
            ...
            if (buildSuccessful) { await addUserBot(cid, appName, newSessionId, false); }
        } catch (buildError) {
...
bot.on('callback_query', async q => {
  ...
  if (action === 'backup') {
      ...
      try {
          ...
          const backupData = { appName: appName, backedUpAt: new Date().toISOString(), userId: cid, sessionId: dbRes.rows[0]?.session_id || null, configVars: configRes.data };
          await fs.writeFile(backupPath, JSON.stringify(backupData, null, 2));
          return bot.editMessageText(`✅ Backup for *${appName}* created successfully!`, { chat_id: cid, message_id: loadingMsg.message_id, parse_mode: 'Markdown' });
      } catch (e) {
          console.error("Backup failed:", e);
          return bot.editMessageText(`❌ Failed to create backup. Error: ${e.message}`, { chat_id: cid, message_id: loadingMsg.message_id });
      }
  }
  if (action === 'dorestore') {
      ...
      try {
          ...
          if (error.response?.status === 404) {
              userStates[cid] = { step: 'RESTORE_SESSION_ID', data: { backupFile: backupFile, appName: appName } };
