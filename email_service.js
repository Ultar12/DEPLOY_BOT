...
async function sendPaymentConfirmation(toEmail, userName, referenceId, appName, botType, sessionId) {
  ...
  const escapedAppName = appName; // Assuming appName is safe
  const escapedSessionId = sessionId; // Assuming sessionId is safe
  ...
  const mailOptions = {
    ...
    html: `
      <div style="background-color: #121212; padding: 20px; font-family: sans-serif; color: #fff; text-align: center; border-radius: 10px;">
        ...
        <p style="font-size: 16px;">We have recorded your unique Session ID for you:</p>
        ...
        <p style="font-size: 12px; color: #aaa; margin-top: 20px;">If you need help, please contact us.</p>
  ...
  try {
    ...
    console.log(`[Email] Payment confirmation sent to ${toEmail} successfully.`);
  } catch (error) {
    console.error(`[Email] Failed to send payment confirmation to ${toEmail}:`, error.message);
    throw error;
  }
}
